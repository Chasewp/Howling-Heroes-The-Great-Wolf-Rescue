...
var iberian_wolf_almanac = preload("res://Assets/Scences/UI/Wolf_Almanac/Book/Iberian Wolf/iberian_wolf_almanac.tscn").instantiate()
...
func _on_area_2d_body_entered(body):
	if body as Players:
		...
		if howling.finished:
			...
			get_wolf.play()
		WolfData._set_alexander_Archipelago_Wolf_Rescue_Status(true)
		#Image
		...
		get_tree().change_scene_to_packed(iberian_wolf_almanac)
		get_tree().root.add_child(iberian_wolf_almanac)
