class_name Mackenzie_River_Wolf 
...
var mackenzie_river_wolf_almanac = preload("res://Assets/Scences/UI/Wolf_Almanac/Book/Mackenzie River Wolf/mackenzie_river_wolf_almanac.tscn").instantiate()
...
func _on_area_2d_body_entered(body):
	if body.is_in_group("player"):
		...
		if howling.finished:
			...
			get_wolf.play()
		WolfData._set_Mackenzie_River_Wolf_Rescue_Status(true)
		#Image
		...
		get_tree().change_scene_to_packed(mackenzie_river_wolf_almanac)
		get_tree().root.add_child(mackenzie_river_wolf_almanac)
